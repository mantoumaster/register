"""BitBrowser Local API client and Playwright CDP connection metadata."""
...
class BitBrowserError(RuntimeError):
    """Raised when the BitBrowser Local API rejects a request."""
...
def proxy_fields_from_url(proxy_url: str) -> Dict[str, Any]:
    """Convert the project's proxy URL into BitBrowser profile fields."""
    if not proxy_url:
        return {
            "proxyMethod": 2,
            "proxyType": "noproxy",
    ...
    value = proxy_url if "://" in proxy_url else f"http://{proxy_url}"
    ...
    return {
        ...
        "proxyUserName": unquote(parsed.username or ""),
        "proxyPassword": unquote(parsed.password or ""),
    }
...
class BitBrowserClient:
    ...
    def create_browser(self, name: str, proxy_url: str = "") -> str:
        payload: Dict[str, Any] = {
            "name": name,
            "workbench": "disable",
            "browserFingerPrint": {},
            **proxy_fields_from_url(proxy_url),
        }
    ...
    def get_or_create_browser(
        ...
        return self.create_browser(name=name, proxy_url=proxy_url)

    def update_browser(self, browser_id: str, **fields) -> None:
        self._post("/browser/update/partial", {"ids": [browser_id], **fields})

    def configure_reusable_browser(
        self,
        browser_id: str,
        name: str,
        proxy_url: str = "",
    ...
    def close_and_clear_browser(
        self, browser_id: str, wait_seconds: float = 5.0
    ) -> None:
        errors = []
        try:
